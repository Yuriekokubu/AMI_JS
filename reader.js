    ...
    .addEventListener('change', function () {
        ...
        fr.onload = function () {
            ...
            for (let [index, elem] of newArray.entries()) {
                ...
                if (elem.length === 72 && firstChar === "H") ArrayIndex.push(index)
            }
            // elem.length == 72 || 84 
            let sliced
            let MergeArray = []
            ...
            const Seperate = MergeArray.map((val, index) => {
                ...
                for (let [index, elem] of val.entries()) {
                    ...
                    elem.length === 163 && firstCharC === "C" && Arr_sorted.push(CustomerFn(elem))
            ...
            Seperate.map((arr, index) => {
                ...
                arr.map((item, position) => {
                    ...
                    if (item.length === 19 && customerID === item[0][2]) {
                        ...
                        RegisterGroup.sort((a, b) => a[2] - b[2] || a[3] - b[3])
                        registerToFill = RegisterGroup.filter((e, i) => e[2] === item[2]);
                        if (registerToFill.length === 18) {
                            customer.Register.push(registerToFill) //flat()
                            RegisterGroup = []
                        }
                    }
                })
        ...
        fr.readAsText(this.files[0], 'UTF-8-BOM');
    })
